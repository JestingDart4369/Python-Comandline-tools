"""
Heartbeat – reports python-cli-tools' health to the API Gateway every 30 s.
If the gateway disables this software entry the whole CLI shuts down
    ...
    def _loop(self):
        while not self._stop.wait(HEARTBEAT_INTERVAL):
            if not self._beat():
                ...
                # inquirer / subprocesses can block the main loop indefinitely
                # → force exit after 1 s grace period
                self._stop.wait(1)
                os._exit(1)
